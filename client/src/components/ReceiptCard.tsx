  ...
  return (
    <View style={[styles.card, { backgroundColor: theme.colors.surface, borderColor: theme.colors.outline }]}>
      <View style={styles.row}>
        <TouchableOpacity onPress={() => receipt.image_uri && onViewImage(receipt.image_uri)}
          disabled={!receipt.image_uri} style={[styles.thumb, { backgroundColor: theme.colors.surfaceVariant }]}>
          {receipt.image_uri
        ...
        <View style={styles.info}>
          <Text variant="titleSmall" numberOfLines={1} style={{ fontWeight: '600' }}>{receipt.merchant}</Text>
        ...
        <View style={styles.right}>
          <Text variant="titleMedium" style={{ fontWeight: '700', color: theme.colors.primary }}>{fmt(receipt.amount)}</Text>
